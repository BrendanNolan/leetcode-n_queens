...
mod leetcode {
    use crate::queen_placer::{self, Square};

    pub fn place_queens(size: usize) -> Vec<Vec<String>> {
        let solutions = queen_placer::place_queens(size);
        convert_solutions_to_leetcode_format(&solutions, size)
    }

    fn convert_solutions_to_leetcode_format(
        solutions: &[Vec<Square>],
        size: usize,
    ) -> Vec<Vec<String>> {
        solutions
            .iter()
            .map(|solution| convert_solution_to_leetcode_format(solution, size))
            .collect()
    }

    fn convert_solution_to_leetcode_format(solution: &[Square], size: usize) -> Vec<String> {
        solution
            .iter()
            .map(|square| convert_square_to_string(square, size))
            .collect()
    }

    fn convert_square_to_string(square: &Square, size: usize) -> String {
        let mut s = String::new();
        for i in 0..size {
            if i == square.column.0 {
                s.push('Q')
            } else {
                s.push('.')
            }
